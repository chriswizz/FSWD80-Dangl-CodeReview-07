...
export class TravelsListComponent implements OnInit {
  ...
  ngOnInit() {
    this.travelsService.getTravels().subscribe( (list) => {
  ...
  filterCondition(travels) {
    return travels.city.toLowerCase().indexOf(this.searchText.toLowerCase()) != -1 || travels.description.toLowerCase().indexOf(this.searchText.toLowerCase()) != -1;
  }
}